...
| No session expiry / stale cleanup | An initiated but never-completed upload holds a session entry in the JVM's heap and chunk files on disk indefinitely | Add `@Scheduled` cleanup that calls `abortUpload(uploadId, session.getUsername())` for sessions older than a configurable TTL (e.g., 24 hours); see `DESIGN.md` technical debt table |
